...
public class WadlApplicationContextTest extends AbstractResourceTester {
    ...
    public WadlApplicationContextTest(final String testName) {
        super(testName);
    }

    @Path("/")
    public static class WadlApplicationContextResource {
    ...
    public void testNullInjectedWhenDisabledWadl() throws Exception {
        final ResourceConfig resourceConfig = new DefaultResourceConfig(WadlApplicationContextResource.class);
        resourceConfig.getFeatures().put(ResourceConfig.FEATURE_DISABLE_WADL, true);

        initiateWebApplication(resourceConfig);

        final WebResource resource = resource("/", false);
        final ClientResponse clientResponse = resource.get(ClientResponse.class);

        assertEquals(200, clientResponse.getStatus());
        assertEquals("ko", clientResponse.getEntity(String.class));
    }
}
