 ...
 */
public class ConfigurationTest extends AbstractGrizzlyServerTester {
    @Path("/")
    public static class RedirectResource {
        @GET
        public Response get(@Context UriInfo ui) {
            return Response.seeOther(
                    ui.getRequestUriBuilder().path("redirected").build()).
                    build();
        }
    ...
    public void testGetWithRedirect() throws Exception {
        startServer(RedirectResource.class);
        WebResource r = Client.create().resource(getUri().build());
        assertEquals("redirected", r.get(String.class));
    }
    
    public void testGetWithNoRedirect() throws Exception {
        startServer(RedirectResource.class);
        Client c = Client.create();
        c.setFollowRedirects(false);
        WebResource r = c.resource(getUri().build());
        assertEquals(303, r.get(ClientResponse.class).getStatus());
    }
}