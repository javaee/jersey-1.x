...
public final class ApacheHttpClientHandler implements ClientHandler
{
    ...
    public ClientResponse handle (ClientRequest cr)
        ...
        try {
            ...
            Boolean proxySet = (Boolean) props.get(ApacheHttpClientConfig.PROPERTY_PROXY_SET);
            if ((proxySet != null) && (proxySet)) {
                HostConfiguration hostConfig = client.getHostConfiguration();
                String proxyHost = (String) props.get(ApacheHttpClientConfig.PROPERTY_PROXY_HOST);
                if (proxyHost == null) {
                    proxyHost = "localhost";
                }

                Integer proxyPort = (Integer) props.get(ApacheHttpClientConfig.PROPERTY_PROXY_PORT);
                if (proxyPort == null) {
                    proxyPort = 8080;
                }
            ...
            HttpState httpState = (HttpState) props.get(DefaultApacheHttpClientConfig.PROPERTY_HTTP_STATE);
            if (httpState != null) {
