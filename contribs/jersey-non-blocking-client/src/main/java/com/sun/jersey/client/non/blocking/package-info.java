 ...
 * <blockquote><pre>
 * DefaultNonBlockingClientConfig config = new DefaultNonBlockingClientConfig();
 * config.getProperties().put(
 *         NonBlockingClientConfig.PROPERTY_AUTH_USERNAME,
 *         "username"
 * );
 * config.getProperties().put(
 *         NonBlockingClientConfig.PROPERTY_AUTH_PASSWORD,
 *         "password"
 * );
 * Client c = NonBlockingClient.create(config);
 * WebResource r = c.resource("http://host/base");
 * String s = r.get(String.class);
 * s = r.post(String.class, s);
 * </pre></blockquote>
